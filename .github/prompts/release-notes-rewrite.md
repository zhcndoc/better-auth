<!-- 提示词结构改编自 sst/opencode（MIT，版权所有 (c) 2025 opencode） -->
...
- 不要添加或删除条目
- 不要修改 PR 链接 `([#NNNN](url))`
- 不要修改包标题（包括任何新包标记，如 `✨`）或其顺序
- 不要修改 `### ❗ Breaking Changes`、`### Features` 或
  `### Bug Fixes` 子标题或其顺序
- 不要修改每个包章节末尾的引用链接
- 不要添加作者署名（`by @username`）
- 不要使用长破折号（—），请使用逗号、冒号或括号
  （例外情况：重大变更标题后的 “ — ” 分隔符允许使用）
- 保持博客文章链接、贡献者章节和完整变更日志
  链接完全不变
- 每个条目保持在单行（标题 + PR 链接），重大变更除外，
  重大变更需要额外增加一行块引用
