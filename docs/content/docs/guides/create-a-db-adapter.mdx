...
<Steps>
  ...
  <Step>
    ...
    <Callout>
      了解更多关于[数据库适配器](/docs/concepts/database#adapters)的信息。
    </Callout>
  </Step>
</Steps>
...
* `debugLog`: 调试日志函数。
* `getModelName`: 用于获取数据库中转换后模型名称的函数。
* `getFieldName`: 用于获取数据库中转换后字段名称的函数。
* `getDefaultModelName`: 用于从架构中获取默认模型名称的函数。
* `getDefaultFieldName`: 用于从架构中获取默认字段名称的函数。
* `getFieldAttributes`: 用于获取特定模型和字段的字段属性的函数。
* `transformInput`: 用于在保存到数据库之前转换输入数据的函数。
* `transformOutput`: 用于从数据库检索后转换输出数据的函数。
* `transformWhereClause`: 用于转换数据库查询 `where` 子句的函数。
...
`consumeOne` 方法以原子方式删除一条匹配的记录并返回该记录，因此两个并发请求无法同时消费同一行。它用于一次性验证挑战等单次使用的凭据。

参数：

* `model`: 要从中消费记录的模型/表名。
* `where`: 用于匹配单条记录的 `where` 子句。
...
<Callout type="warn">
  `consumeOne` 是必需的。Better Auth 不提供 `findMany` 加 `deleteMany` 的回退方案，因为在没有原生原子删除并返回操作的情况下，这种形式无法保证各适配器之间的单次使用语义。
</Callout>

### `incrementOne` 方法

`incrementOne` 方法通过应用数值增量和可选的设置值，以原子方式更新一条匹配的记录，然后返回更新后的行。它用于速率限制、配额和单一获胜者状态转换等受保护的计数器。

参数：

* `model`: 要更新的模型/表名。
* `where`: 应仍然匹配，增量操作才会应用的 `where` 子句。
* `increment`: 要应用的数值增量，例如 `{ count: 1 }` 或 `{ remaining: -1 }`。
* `set`: 要在同一原子操作中设置的可选字段。

**Returns** `Promise<T | null>`: 更新后的记录；如果没有匹配的行则为 `null`。实现时最多只能更新一条匹配行。
...
<Callout type="warn">
  `incrementOne` 是必需的。Better Auth 将其用作比较并交换原语：`where` 子句是保护条件的一部分，因此并发请求无法全部更新过时的快照。
</Callout>
...
用于启用适配器的调试日志。你可以传入布尔值，也可以传入包含以下键的对象：`create`、`update`、`updateMany`、`findOne`、`findMany`、`delete`、`deleteMany`、`consumeOne`、`incrementOne`、`count`。
如果任意键为 `true`，则会为相应方法启用调试日志。
