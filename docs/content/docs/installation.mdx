  ...
  <Step>
    ...
    <Tabs items={["sqlite", "postgres", "mysql"]}>
      ...
      <Tab value="prisma">
        ...
        请参阅 [Prisma 适配器指南](/docs/adapters/prisma)，了解 Prisma 7
        驱动适配器和生成客户端的设置。
      </Tab>
    ...
    如果你的数据库未在上方列出，请查看我们支持的其他 [数据库](/docs/adapters/other-relational-databases) 了解更多信息，
    或使用受支持的 ORM 之一。

    <Callout>
      如果你正在使用数据库适配器（Drizzle、Prisma、MongoDB 或社区适配器），
      可以考虑从 `better-auth` 改为从 `better-auth/minimal` 导入 `betterAuth` 以减小包体积。
      有关更多信息，请参阅 [包体积优化](/docs/guides/optimizing-for-performance#bundle-size-optimization)。
    </Callout>
  </Step>
