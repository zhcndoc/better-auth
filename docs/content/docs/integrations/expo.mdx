...
<Steps>
  <Step>
    ...
    要开始使用，请先查看我们的 [安装](/docs/installation) 指南，了解如何在服务器上配置 Better Auth。如果你愿意，也可以查看 [完整示例](https://github.com/better-auth/examples/tree/main/expo-example)。
  ...
  <Step>
    ...
    更多关于可信来源的信息，请参阅 [可信来源文档](/docs/reference/options#trustedorigins)。
...
<Callout type="info">
  在原生平台（iOS/Android）上，`signIn.social` 不会自动导航。它完成后请自行处理导航。请注意，[底层浏览器行为因平台而异](https://docs.expo.dev/versions/latest/sdk/webbrowser/#webbrowseropenauthsessionasyncurl-redirecturl-options)。
</Callout>
...
**storage**：用于缓存会话数据和 Cookie 的 SecureStore 兼容存储。
