...
<Callout>
  我们强烈建议，如果你需要对某个端点进行自定义调整，请使用钩子，而不是在 Better Auth 之外创建另一个端点。
</Callout>
...
* `ctx.context.password.verify`: 用于验证给定的 `password` 与 `hash` 是否匹配。

#### 适配器（Adapter）

适配器暴露了 Better Auth 使用的适配器方法，包括 `findOne`、`findMany`、`create`、`delete`、`update` 和 `updateMany`。通常推荐使用你的 ORM 中的实际 `db` 实例，而非该适配器。

#### 内部适配器

这些是对数据库执行特定操作的调用，如 `createUser`、`createSession`、`updateSession` 等。
