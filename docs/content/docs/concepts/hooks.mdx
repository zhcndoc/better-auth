...
<Callout>
  每个钩子（`before` / `after`）只接受一个中间件函数，而不是数组。要为不同端点运行逻辑，请在这个单一函数内部基于 `ctx.path` 进行分支判断。
</Callout>
...
* 设置 Cookie：`ctx.setCookie` 或 `ctx.setSignedCookie`。
* 获取 Cookie：`ctx.getCookie` 或 `ctx.getSignedCookie`。
