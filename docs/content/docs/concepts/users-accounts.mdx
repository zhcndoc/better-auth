...
* **关联社交账户**: 使用客户端的 `linkSocial` 方法将一个社交提供者关联到用户的账户。
  ...
  * 当实现自定义 OAuth 流程时
  ...
  默认情况下，关联账户不会更改现有用户资料。启用 `updateUserInfoOnLink` 后，每次关联账户时都会将提供者的资料复制到用户上。同步的字段与注册时持久化的字段相同（`name`、`image` 以及 `mapProfileToUser` 添加的任何字段）。用户的 `email` 和 `emailVerified` 在关联时绝不会更改，因此关联提供者不会重新绑定账户身份。
