...
| ------------------ | ------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------- |
| Apple              | 每次登录（除首次外）（Apple 只会在首次同意时发出 `email`）                                                                                | `profile.sub`（对每个 Apple Team 稳定）               | 可靠；中继地址也会被标记为已验证                                                                                                       |
| Discord            | 仅有手机号的账号；未授予 `email` scope                                                                                          | `profile.id`（雪花 ID）                            | 可靠（有专门的 `verified` 字段）                                                                                                 |
| Facebook           | 即使授予了 `email` 权限，文件中也没有有效邮箱                                                                                         | `profile.id`（应用作用域）                            | 未知：Graph API 不提供按邮箱的验证标志                                                                                               |
| GitHub             | 用户将邮箱设为私密；GitHub App 缺少 “Email addresses” 权限                                                                        | `profile.id`（数字）                               | 可靠                                                                                                                     |
| LinkedIn           | 成员没有已确认邮箱；未授予 `email` scope                                                                                         | `profile.sub`（每个应用成对唯一）                        | 存在时可靠                                                                                                                  |
| Microsoft Entra ID | 未管理用户没有 `mail` 属性，除非将 `email` 配置为 [可选声明](https://learn.microsoft.com/en-us/entra/identity-platform/optional-claims) | `profile.oid` 加 `profile.tid`（或 `profile.sub`） | **不可信**：Microsoft [明确警告](https://learn.microsoft.com/en-us/entra/identity-platform/id-token-claims-reference) 不要将其用于授权 |
| Roblox             | 默认的 Roblox 资料流程不会返回邮箱；Better Auth 目前回退到 `preferred_username`                                                        | `profile.sub`（Roblox 用户 ID）                    | 默认资料流程下未知                                                                                                              |
...
针对无邮箱用户的一等支持——使用稳定的 `(providerId, accountId)` 对作为身份键（与 [OpenID Connect Core §5.7](https://openid.net/specs/openid-connect-core-1_0.html#ClaimStability) 一致）——已在 [#9124](https://github.com/better-auth/better-auth/issues/9124) 中跟踪。
