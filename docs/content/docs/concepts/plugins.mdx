...
Better Auth 封装了另一个名为 <Link href="https://github.com/bekacru/better-call"> Better Call </Link> 的库来创建端点。Better Call 是一个简单的 TypeScript Web 框架，由与 Better Auth 相同的团队打造。
...
* 确保端点路径使用 kebab-case
* 确保端点仅使用 `POST` 或 `GET` 方法。
* 任何修改数据的函数都应使用 `POST` 方法。
* 任何获取数据的函数都应使用 `GET` 方法。
* 确保使用 `createAuthEndpoint` 函数创建 API 端点。
* 确保路径具有唯一性，以避免与其他插件发生冲突。如果使用常见路径，请将插件名称作为路径前缀。（使用 `/my-plugin/hello-world`，而不是 `/hello-world`。）
    ...
    return {
        ...
        schema: {
            myTable: {
                ...
                modelName: "myTable" // 如果需要与键名不同，可以指定，否则可以省略
            }
        }
    } satisfies BetterAuthPlugin
}
...
`unique`: 字段是否必须唯一。（默认：`false`）

对于跨多个字段的索引，请向表中添加 `indexes` 数组。即使字段具有自定义数据库列名，索引字段也使用 `fields` 中的逻辑字段键。
...
每个索引最多可以包含 16 个字段。当每个字段元组最多只能标识一行时，将 `unique: true` 设置为唯一索引。唯一的表级索引中的每个字段都必须是必填字段，以确保跨数据库的 null 处理保持一致。您还可以提供一个模式范围的 `name`，长度最多为 63 个 UTF-8 字节，以控制数据库索引名称。名称必须以字母或下划线开头，且只能包含字母、数字和下划线。名称比较不区分大小写，不能与表或其他索引冲突。CLI 会将表级索引包含在 SQL 迁移以及生成的 Drizzle 或 Prisma 模式中。MongoDB 适配器会在对集合执行首次索引写入前创建这些索引。

`references`：如果字段是对另一张表的引用。（可选）它接受一个包含以下属性的对象：

* `model`：要引用的表名。
* `field`：要引用的字段名。
* `onDelete`：所引用记录被删除时要执行的操作。（默认：`cascade`）

**其他 Schema 属性**

`disableMigration`: 表是否不应进行迁移。（默认：`false`）
...
* **正常继续**：如果您不返回任何内容，请求将按正常流程继续。
* **中断请求**：要停止请求并发送响应，请返回一个包含 `response` 属性的对象，该属性包含一个 `Response` 对象。
* **修改请求**：您还可以返回一个修改后的 `request` 对象，以在请求发送前更改请求。
...
* **修改响应**：您可以返回一个修改后的响应对象以更改响应，然后再发送给客户端。
