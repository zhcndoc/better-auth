---
title: 会话管理
description: 了解 Better Auth 中的会话管理，包括会话过期、新鲜度、Cookie 缓存策略、次级存储、无状态会话以及自定义会话响应。
---

Better Auth 使用传统的基于 Cookie 的会话管理机制。会话数据存储在 Cookie 中，并在每个请求时发送到服务器。服务器随后验证会话，若会话有效则返回用户数据。

主 `session_token` Cookie 仍然是服务器端的会话标识符。如果启用 `session.cookieCache`，Better Auth 还会写入一个单独的 `session_data` Cookie，用于短时缓存会话数据。此缓存 Cookie 与 JWT 插件的 `/token` 端点和 `set-auth-jwt` 标头输出相互独立。
...
部分 Better Auth 接口要求会话必须是 **新鲜的**。如果会话的 `createdAt` 在 `freshAge` 限制范围内，则该会话被视为新鲜。默认情况下，`freshAge` 设置为 **1 天**（60 \* 60 \* 24）。
...
有关自定义会话响应的说明，请参阅 [自定义会话响应](#customizing-session-response) 部分。
...
每次调用 `useSession` 或 `getSession` 时访问数据库并不理想，尤其是会话不常变化时。Cookie 缓存通过将会话数据存储在短时效、已签名的 Cookie 中来解决这个问题——类似使用 JWT 访问令牌和刷新令牌的模式。
...
* **`compact`**（默认）：使用 base64url 编码和 HMAC-SHA256 签名。不包含 JWT 规范开销，格式最紧凑。最适合关注性能和体积的场景。
* **`jwt`**：标准 JWT。默认使用 HMAC-SHA256（HS256）和你的 Better Auth 密钥。令牌经过签名但未加密——任何人都可以读取，但无法篡改。如果你需要能够通过 JWT 插件的 JWKS 验证的会话缓存 Cookie，请安装 `jwt()` 插件并设置 `sessionCookieCache: true`，以使用其非对称签名密钥。
* **`jwe`**：使用 JWE（JSON Web Encryption）以及 A256CBC-HS512 和 HKDF 密钥派生。令牌经过完全加密——既无法读取，也无法篡改。安全性最高，但体积最大。

**比较：**

| 策略        | 体积 | 安全性     | 可读性 | 互操作性 | 适用场景               |
| --------- | -- | ------- | --- | ---- | ------------------ |
| `compact` | 最小 | 良好（已签名） | 是   | 否    | 性能敏感、内部使用          |
| `jwt`     | 中等 | 良好（已签名） | 是   | 是    | 需要 JWT 兼容或与第三方系统集成 |
...
* **使用 `compact`**：当你需要最高性能和最小 Cookie 体积时使用。最适合 Cookie 仅由 Better Auth 在内部使用的大多数应用。
* **使用 `jwt`**：当你需要与外部系统兼容 JWT，或希望使用可由第三方工具验证的标准 JWT 令牌时使用。令牌可读（以 base64 编码的 JSON 形式），但无法被篡改。如果希望 Cookie 缓存 JWT 通过其 JWKS 而非共享密钥进行验证，请在 JWT 插件上启用 `sessionCookieCache`。
* **使用 `jwe`**：当你需要最高安全性并希望对客户端隐藏会话数据时使用。令牌经过完全加密，没有密钥就无法读取。适用于敏感数据或合规性要求。

### 基于 JWKS 的 Cookie 缓存 JWT

如果你希望 `session_data` Cookie 通过 JWT 插件的 JWKS 端点而不是共享密钥进行验证，请启用 JWT 插件，并明确选择基于 JWKS 的签名：
...
这只会影响 `session_data` Cookie。主要的 `session_token` Cookie 仍然是不透明的会话标识符，不会通过 JWKS 暴露。Cookie 缓存 JWT 还使用与 JWT 插件 `/token` 端点不同的令牌配置，因此这些令牌不可互换。

如果你希望在获取会话时禁用从 Cookie 缓存返回会话，可以传入 `disableCookieCache:true`，这将强制服务器从数据库获取会话，同时刷新 Cookie 缓存。
...
默认情况下，如果在身份验证配置中提供了[次级存储](/docs/concepts/database#secondary-storage)，会话将存储于次级存储中。
...
当会话被撤销时，它会从次级存储中移除；但是，如果启用 `preserveSessionInDatabase`，会话记录将保留在数据库中，而不是被删除。保留的记录会被标记为已结束（其 `expiresAt` 被设置为撤销时间），因此不会被恢复为活跃会话，并且与其绑定的任何令牌（例如 OAuth Provider 访问令牌）都会被视为已结束。
...
export const auth = betterAuth({
    ...
    account: {
        ...
        storeAccountCookie: true, // OAuth 流程后将提供方账户数据存储在加密 Cookie 中
    }
});
```
...
在无状态 OAuth 流程中，`storeAccountCookie` 会将提供方账户数据（包括 OAuth 令牌材料）存储在加密的 `account_data` Cookie 中。当账户 Cookie 包含刷新令牌和已知的访问令牌过期时间时，`getAccessToken({ useAccountCookie: true })` 可以刷新已过期的提供方访问令牌。令牌刷新响应会设置更新后的账户 Cookie，因此服务器端集成必须将返回的 `Set-Cookie` 标头转发给浏览器。

Better Auth 会对过大的账户 Cookie 进行分块，但浏览器和代理仍可能限制总 Cookie 或请求头大小。对于发放大型 JWT 的提供方，或需要持久化令牌存储的生产流程，请使用由数据库支持的账户存储。
