...
在无状态 OAuth 流程中，`storeAccountCookie` 会将提供方账户数据（包括 OAuth 令牌材料）存储在加密的 `account_data` Cookie 中。当 `getAccessToken` 发现账户 Cookie 中包含刷新令牌和已知的访问令牌过期时间时，可以刷新过期的提供方访问令牌。令牌刷新响应会设置更新后的账户 Cookie，因此服务器端集成必须将返回的 `Set-Cookie` 头转发给浏览器。
