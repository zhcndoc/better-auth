...
<Callout>
  如果服务器无法返回响应对象，你需要手动解析并设置 Cookie。但对于 Next.js 等框架，我们提供了[一个插件](/docs/integrations/next#server-action-cookies)来自动处理
</Callout>
...
你也可以使用社交提供商提供的 `idToken` 或 `accessToken` 进行认证，而无需将用户重定向到提供商的网站。更多详情请参阅社交提供商文档。
...
<Callout>
  更多详情请查看[会话管理](/docs/concepts/session-management)文档
</Callout>
...
<Steps>
  ...
  <Step>
    ...
    <Callout>
      如果你更喜欢手动添加架构，可以查看[双因素插件](/docs/plugins/2fa#schema)文档中所需的架构
    </Callout>
  </Step>
  ...
  <Step>
    下一步：查看 <Link href="/docs/plugins/2fa">双因素插件文档</Link>
  </Step>
</Steps>
