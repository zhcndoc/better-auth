...
* `clientId`: 提供商的 OAuth 客户端 ID
* `clientSecret`: 提供商的 OAuth 客户端密钥
* `clientKey`: 客户端密钥（某些提供商如 TikTok 会用它替代 clientId）（可选）
* `redirectURI`: OAuth 回调的自定义重定向 URI（可选）
* `scope`: 要请求的额外 OAuth 权限范围（可选）
