...
* `clientKey`: 客户端密钥（某些提供商如 TikTok 使用它而不是 clientId）（可选）
* `redirectURI`: OAuth 回调的自定义重定向 URI（可选）
* `scope`: 要请求的额外 OAuth 作用域（可选）
* `mapProfileToUser`: 将提供商资料映射为用户的自定义函数（可选）
* `disableSignUp`: 禁用新用户注册（可选）
* `disableImplicitSignUp`: 禁用新用户的隐式注册（可选）
* `overrideUserInfoOnSignIn`: 登录时使用提供商用户信息覆盖用户信息（可选）
* `prompt`: 授权码请求使用的 prompt（`"select_account"`、`"consent"`、`"login"`、`"none"`、`"select_account consent"`）（可选）
* `responseMode`: 使用的响应模式（`"query"`、`"form_post"`）（可选）
* `getUserInfo`: 从提供商获取用户信息的自定义函数（可选）
* `refreshAccessToken`: 刷新令牌的自定义函数。接收已存储的刷新令牌并返回新的 OAuth 令牌。（可选）
* `verifyIdToken`: 验证 ID token 的自定义函数。接收 `(token, nonce?, ctx?)`，其中 `ctx` 是请求端点上下文（可选）。设置后会替换提供商内置的验证。
* `disableIdTokenSignIn`: 禁用使用从客户端发送的 ID token 登录（可选）
...
Better Auth 会对过大的账号 cookie 进行分片，但浏览器和代理仍可能限制 cookie 或 header 的总大小。对于会签发较大 JWT 的提供商，或需要持久令牌存储的生产流程，优先使用基于数据库的账号存储。
...
* `generateId`: 控制记录 ID 的生成方式。接受自定义函数、`false`、`"serial"` 或 `"uuid"`（默认：[随机 base62 字符串](https://github.com/better-auth/better-auth/blob/main/packages/core/src/utils/id.ts)）。更多信息请参阅[数据库文档](/docs/concepts/database#id-generation)。
