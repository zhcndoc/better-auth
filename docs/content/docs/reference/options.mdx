...
Better Auth 配置的所有可用选项列表。详见 [Better Auth 选项](https://github.com/better-auth/better-auth/blob/main/packages/core/src/types/init-options.ts)。
...
当你的应用可以通过多个域名访问时，例如预览部署、分支环境或多个生产主机，请使用对象形式。Better Auth 会从 `Host` 请求头以及请求 URL 中获取主机，然后根据 `allowedHosts` 对其进行验证，并根据匹配结果构造请求专属的 baseURL。除非你显式设置 `advanced.trustedProxyHeaders: true`，否则会忽略转发的主机和协议请求头。
...
* `allowedHosts`：接受的主机模式列表。支持精确匹配（`myapp.com`）、通配符（`*.vercel.app`、`preview-*.myapp.com`）和端口通配符（`localhost:*`）。会自动添加到 [`trustedOrigins`](#trustedorigins)（localhost 条目同时添加 `http` 和 `https`）
* `protocol`：用于 URL 构造的协议。`"http"`、`"https"` 或 `"auto"`（默认值）。仅当 `advanced.trustedProxyHeaders` 为 `true` 时，`"auto"` 才会使用 `x-forwarded-proto`，然后依次使用请求 URL、适用于本地回环主机的 HTTP，否则使用 HTTPS。也会影响 cookie 的 `Secure` 标志：`"https"` → 安全，`"http"` → 不安全，`"auto"` 或未设置 → `NODE_ENV === "production"`。可通过 `advanced.useSecureCookies` 覆盖
* `fallback`：当没有可用的请求主机或传入主机不匹配时使用的 URL。未设置时，无法解析或未知的主机会抛出错误。其来源也会添加到 `trustedOrigins`
...
有关部署模式和端到端示例，请参阅 [动态 Base URL 指南](/docs/guides/dynamic-base-url)。
...
export const auth = betterAuth({
	trustedOrigins: async (request) => {
		// 在初始化和调用 auth.api 时，request 为 undefined
		if (!request) {
...
<Callout type="info">
  ...
  * 对于 `http://` 和 `https://` URL，模式会匹配完整来源。路径和查询字符串会被忽略。
  * 对于自定义协议（如 `exp://` 或 `myapp://`），通配符模式会针对包含路径在内的完整 URL 进行匹配。非通配符模式会根据协议和权限进行匹配：不包含主机的条目（如 `myapp://` 或 `exp://`）会信任该协议的所有主机，而包含主机的条目（如 `myapp://callback`）必须精确匹配该主机（因此不会被 `myapp://callback.attacker.tld` 满足）。
  * 分隔符是 `/`（正斜杠）。这意味着 `http://*.example.com` 同时匹配 `http://api.example.com` 和 `http://api.app.example.com`。
</Callout>
...
阅读更多关于 [数据库](/docs/concepts/database)。
...
阅读更多关于 [二级存储](/docs/concepts/database#secondary-storage)。
...
* `clientKey`: 客户端密钥（某些提供商（如 TikTok）使用它代替 clientId）（可选）
* `redirectURI`: OAuth 回调的自定义重定向 URI（可选）
* `scope`: 要请求的其他 OAuth 作用域（可选）
* `mapProfileToUser`: 将提供商资料映射到用户的自定义函数（可选）
* `disableSignUp`: 禁止新用户注册（可选）
* `disableImplicitSignUp`: 禁止新用户隐式注册（可选）
* `overrideUserInfoOnSignIn`: 登录时使用提供商用户信息覆盖用户信息（可选）
* `requireEmailVerification`: 在创建会话前要求验证此提供商的邮箱。按提供商单独启用，并且独立于 `emailAndPassword.requireEmailVerification`。仅对报告可信 `email_verified` 信号的提供商启用（可选）
* `prompt`: 授权码请求使用的提示方式（`"select_account"`、`"consent"`、`"login"`、`"none"`、`"select_account consent"`）（可选）
* `responseMode`: 使用的响应模式（`"query"`、`"form_post"`）（可选）
* `getUserInfo`: 从提供商获取用户信息的自定义函数（可选）
* `refreshAccessToken`: 刷新令牌的自定义函数。接收存储的刷新令牌并返回新的 OAuth 令牌（可选）
* `verifyIdToken`: 验证 ID 令牌的自定义函数。接收 `(token, nonce?, ctx?)`，其中 `ctx` 是请求端点上下文（可选）。设置后会替代提供商内置的验证逻辑
* `disableIdTokenSignIn`: 禁止使用从客户端发送的 ID 令牌登录（可选）
* `disableDefaultScope`: 禁用提供商的默认作用域（可选）
...
export const auth = betterAuth({
	account: {
		...
		storeStateStrategy: "database", // 在验证存储中存储 OAuth 状态负载
		storeAccountCookie: true, // 在 OAuth 流程后将提供商账号数据存储在加密 cookie 中（对无数据库流程很有用）
		accountLinking: {
			enabled: true,
...
当配置了数据库或 `secondaryStorage` 时，默认使用 `"database"`；只有在两者都未设置（完全无状态方案）时才使用 `"cookie"`。使用 `secondaryStorage` 时，OAuth 状态会自动存储在那里；你不再需要手动设置。

### `storeAccountCookie`

在 OAuth 流程后将提供商账号数据存储在加密 cookie 中。这对无数据库流程很有用，因为此时必须在没有账号表的情况下获取 OAuth 令牌材料，例如 access token、refresh token、ID token、作用域和令牌过期信息。

* 默认值：`false`
* 如果未提供数据库，会自动设置为 `true`

账号 cookie 的最大有效期为五分钟，并会在 Better Auth 写入更新后的提供商账号数据时刷新，例如在 `getAccessToken` 或 `/refresh-token` 刷新提供商令牌后。发生这种情况时，如果是在服务端代码中，请将返回的 `Set-Cookie` 头转发给浏览器，以保留刷新的账号 cookie。

在 `getAccessToken`、`refreshToken` 或 `accountInfo` 中传入 `useAccountCookie: true`，即可让已签名 cookie 选择账号。这些 API 要求显式提供 `accountId` 或 `useAccountCookie: true`；两者都省略则无效。

Better Auth 会将过大的账号 cookie 分块，但浏览器和代理仍可能限制 cookie 或请求头的总大小。对于会签发大型 JWT 的提供商，或需要持久化令牌存储的生产流程，建议使用基于数据库的账号存储。

启用此选项后，你可以在 hooks 或 middleware 中通过 `better-auth/cookies` 的 `getAccountCookie` 读取解密后的账号 cookie：
...
* `disableImplicitLinking`: 禁用 OAuth 登录时的自动账号关联。当为 `true` 时，具有相同邮箱的现有用户在 OAuth 登录时会返回 `account_not_linked`，而不会被隐式关联（即使邮箱已验证或属于 `trustedProviders`）。新用户仍然可以通过 OAuth 注册，已认证用户也仍然可以通过 `linkSocial()` 显式关联提供商。（默认：`false`）
* `trustedProviders`: 受信任提供商列表。可以是静态数组，也可以是根据请求动态返回提供商的异步函数（类似 `trustedOrigins`）
* `allowDifferentEmails`: 允许用户使用不同的电子邮件地址关联账号
* `allowUnlinkingAll`: 允许用户解除所有账号关联
* `updateUserInfoOnLink`: 关联时，将提供商资料（`name`、`image` 以及任何 `mapProfileToUser` 字段）复制到本地用户。不会更改本地 `email` 和 `emailVerified`。（默认：`false`）
...
* `ipAddress`: 用于速率限制和会话跟踪的 IP 地址配置
  * `disableIpTracking`: 禁用 IP 地址跟踪。
  * `ipAddressHeaders`: 用于读取客户端 IP 地址的受信任请求头名称。
  * `trustedProxies`: 反向代理 IP 或 CIDR 范围。转发链会从右向左遍历，跳过受信任的跳点，首个不受信任的地址会被用作客户端 IP。
* `useSecureCookies`: 默认情况下，生产环境中的 cookie 是安全的。将其设为 `true` 可在所有环境中强制使用 `Secure` cookie 属性。
* `disableCSRFCheck`: 禁用所有 CSRF 防护，包括来源头验证和 Fetch Metadata 检查（⚠️ 安全风险）
* `disableOriginCheck`: 禁用 `callbackURL`、`redirectTo` 以及其他重定向 URL 的 URL 验证（⚠️ 安全风险）
* `crossSubDomainCookies`: 配置可跨子域共享的 cookie
* `cookies`: 自定义 cookie 名称和属性
* `defaultCookieAttributes`: 所有 cookie 的默认属性
* `cookiePrefix`: cookie 前缀
* `database`: 数据库配置选项
* `backgroundTasks`: 配置延迟操作的后台任务处理
* `skipTrailingSlashes`: 在路由匹配中跳过尾随斜杠验证。（默认：`false`）
* OAuth 状态配置选项（`storeStateStrategy`、`skipStateCookieCheck`）现在属于 `account` 选项

<h3 id="advanced-database"><code>database</code></h3>

为 ID 生成、findMany 查询和数据库连接设置自定义策略。

* `generateId`: 控制记录 ID 的生成方式。接受自定义函数、`false`、`"serial"` 或 `"uuid"`（默认值：[随机 base62 字符串](https://github.com/better-auth/better-auth/blob/main/packages/core/src/utils/id.ts)）。更多信息请参阅[数据库文档](/docs/concepts/database#id-generation)。
* `defaultFindManyLimit`: `findMany` 适配器方法返回的记录默认最大数量（默认值：`100`）
* `joins`: 为支持连接的适配器启用数据库连接。禁用时（默认值），相关数据会通过单独查询获取。更多信息请参阅[数据库文档](/docs/concepts/database#joins)。（默认值：`false`）
...
一些示例包括 Vercel 上来自 `@vercel/functions` 的 `waitUntil`，以及 Cloudflare Workers 上来自 `cloudflare:workers` 的 `waitUntil`。更多详情请参阅下面的示例。

<Callout type="warn">
  启用该功能会引入最终一致性，即响应返回的是乐观数据，数据库尚未更新。仅当你的应用能接受这种权衡以换取延迟优化时启用。
</Callout>
