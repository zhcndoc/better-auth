...
<Callout type="warn">
  ...
  * **使用会话头信息时：** 组织会为已认证的会话用户创建。`userId` 字段会被**静默忽略**。
  * **不使用会话头信息时（仅服务端）：** 组织会为 `userId` 指定的用户创建。

  **对于管理员：** 若要代表其他用户创建组织，必须在服务端调用 API，且**不要**传递会话头信息。
</Callout>
...
要获取不包含成员或邀请的组织元数据，请使用 `getOrganization`。
默认情况下，如果不传递任何属性，它将使用活跃组织。

当您只需要 `id`、`name`、`slug`、`logo` 和 `metadata` 等字段时，优先使用此方法，而不是 `getFullOrganization`。
...
<APIMethod path="/organization/invite-member" method="POST" requireSession>
  ...
  type createInvitation = {
      ...
      /**
       * 要分配给用户的角色。可以是 `admin`、`member`、`owner`
       */
...
默认情况下，接受邀请需要使用邮件链接中的邀请 ID，并且当前已登录会话的邮箱必须与邀请邮箱匹配。当 Better Auth 使用内置的不可预测邀请 ID 时，包括默认生成器或 `advanced.database.generateId: "uuid"`，这对于正常的邮件邀请流程已经足够。如果邀请 ID 是外部可控或可预测的，例如 `advanced.database.generateId: "serial"` / `false` 或自定义 ID 生成方式，则 Better Auth 还会要求进行邮箱验证，除非您显式将 `requireEmailVerificationOnInvitation` 设为 `false`。

建议将 `requireEmailVerificationOnInvitation` 设为 `true`，以采用更严格的安全策略。当邀请 ID 可能会被邀请用户邮箱之外的人看到、当应用向成员展示组织邀请列表、当使用自定义邀请投递方式，或允许未验证邮箱/密码会话且组织成员资格较敏感时，尤其建议这样做。在登录前要求邮箱验证，可以在流程更早阶段提供相同的所有权证明。
...
如果您想了解用户如何拒绝邀请，可以查看 [拒绝邀请部分](#reject-invitation)。
...
<Callout type="info">
  通过 ID 获取邀请遵循与按 ID 接受邀请相同的邮箱验证策略。
</Callout>
...
响应中包含邀请 ID。请将这些 ID 视为具有操作能力的邀请链接。如果能够列出组织邀请的用户不应使用这些 ID 在未验证的收件人会话中执行操作，请启用 `requireEmailVerificationOnInvitation`，或在应用层为该端点添加权限检查。
...
客户端侧的 `listUserInvitations` 调用要求会话用户的邮箱已验证。此端点会从会话邮箱中枚举待处理的邀请 ID，因此仅靠邮箱字符串匹配不足以证明所有权。

在服务端，您可以通过查询参数传入用户的邮箱地址。
...
将指定团队设为当前活跃组织的当前活跃团队。如果 `teamId` 为 `null`，则取消当前活跃团队。
...
列出用户所属的所有团队。默认为当前用户，并返回该用户所属每个组织中的团队。

* 传入 `userId` 可列出其他成员所属的团队。此操作受目标组织中 `member:update` 权限的限制。
* 传入 `organizationId` 可将结果限定为单个组织，无需切换会话的活跃组织。省略时，查询其他用户将使用会话的活跃组织。

<APIMethod path="/organization/list-user-teams" method="GET" requireSession>
  ```ts
  type listUserTeams = {
    /**
     * 要列出其团队的用户 ID。默认为当前会话用户。
     */
    userId?: string
    /**
     * 用于限定团队列表范围的组织 ID。对自身进行查询时若省略，则返回该用户所属每个组织中的团队。查询其他用户时，则回退到会话的活跃组织。
     */
...
* `maximumTeams`: 限制每个组织的团队数量
  ...
  `maximumMembersPerTeam` 在接受团队邀请、将现有组织成员添加到团队，以及通过 `teamId` 添加新组织成员时强制执行。
...
**requireEmailVerificationOnInvitation**: `boolean | undefined` - 是否在受邀者调用带有邀请 ID 的邀请相关接口（`acceptInvitation`、`rejectInvitation`、`getInvitation`）之前要求邮箱验证。若未设置，Better Auth 会对内置的、不可见的邀请 ID 保持正常的邮件邀请流程，包括默认生成器以及 `advanced.database.generateId: "uuid"`。对于外部可控或可预测的邀请 ID，例如 `advanced.database.generateId: "serial"` / `false` 或自定义 ID 生成方式，则会要求验证。当邀请 ID 可能在受邀用户邮箱之外可见、当组织邀请列表会向成员公开，或者你希望已验证邮箱作为按 ID 执行邀请操作的所有权证明时，请将此选项设为 `true`。客户端侧的 `listUserInvitations` 始终需要已验证的会话邮箱，因为它会从会话邮箱中枚举邀请 ID。
