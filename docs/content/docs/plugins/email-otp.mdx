...
<APIMethod path="/email-otp/send-verification-otp" method="POST">
  ...
  type sendVerificationOTP = {
      ...
      /**
       * OTP 的类型。`sign-in`、`email-verification` 或 `forget-password`。 
       */
...
<Callout>
  如果用户未注册，他们将自动注册。为新用户也会接受已配置的[附加字段](/docs/concepts/typescript#additional-fields)。若要阻止自动注册，请在[选项](#选项)中将 `disableSignUp` 设为 `true`。
</Callout>

<Callout>
  当签名 OTP 确认了一个已有账户且该账户的电子邮件从未验证过时，该账户上任何现有密码都会被移除，其会话也会被撤销。用户会通过 OTP 登录，并可通过密码重置设置新密码。这样可确保由 OTP 证明的邮箱所有权作为该账户的事实来源。
</Callout>
...
<Callout type="warn">
  `/forget-password/email-otp` 端点已弃用。请改用 `/email-otp/request-password-reset`。
</Callout>
...
<APIMethod path="/email-otp/request-email-change" method="POST" requireSession>
  ...
  type requestEmailChangeEmailOTP = {
      ...
      /**
       * 发送到当前邮箱的 OTP。 当 `changeEmail.verifyCurrentEmail` 选项设置为 `true` 时必需。
       */
      otp?: string = "123456"
  }
  ```
...
<APIMethod path="/email-otp/send-verification-otp" method="POST">
  ...
  type sendVerificationOTP = {
      ...
      email: string = "user@example.com"
      /**
       * OTP 的类型。必须为 `email-verification`，用于确认邮箱更改。
       */
