...
当 `createSession` 为 `true` 时，成功验证会设置会话 cookie，并返回与已注册 passkey 关联的 `session` 和 `user`。如果您想避免在 WebAuthn 流程成功之前创建用户，请在 `registration.afterVerification` 中创建或加载用户，并返回其 `userId`。
...
<APIMethod path="/passkey/add-passkey" method="POST" isClientOnly>
  ...
  type addPasskey = {
      ...
      context?: string
      /**
       * 成功注册 passkey 后创建会话。启用后，响应中会包含 session 和 user。
       */
...
当用户注册 passkey 时如果未为其命名，存储的 `name` 会保持为空。您可以在 UI 中显示一个友好的默认名称，该名称可根据创建该凭证的认证器推导得出。

每一条 passkey 记录都带有一个 `aaguid`，即认证器*型号*的标识符（例如 Google Password Manager 或 1Password）。Better Auth 会在注册时存储它，并在 `listPasskeys` 中返回，因此您可以在渲染 passkey 时确定显示标签。由于解析发生在读取时，更新后的提供商列表也会应用于已存在的 passkey。

该插件为最常见的认证器附带了一个小型、尽力而为的查找表：
...
内置列表刻意保持较小，并不具有权威性。许多认证器都缺失，而且注重隐私的平台会报告一个全零的 AAGUID，从而无法解析出任何结果（Apple 设备在默认的 `attestation: "none"` 流程下就是这样）。您可以使用导出的 `commonAuthenticatorNames` 映射来扩展它，或者对照社区维护的数据源以获得完整覆盖：
...
要在注册时于服务端设置默认标签，请在 `registration.afterVerification` 中返回一个 `name`。AAGUID 可通过 `verification.registrationInfo.aaguid` 获取，而且客户端提供的名称始终优先：
