...
<APIMethod path="/passkey/add-passkey" method="POST" isClientOnly>
  ...
  type addPasskey = {
      ...
      */
      ...
      /**
       * 您也可以指定要注册的认证器类型。默认行为允许平台和跨平台的 passkey。
      */
      ...
      /**
       * 用于 passkey-first 注册流程的可选上下文。会转发给 `registration.resolveUser`。
       */
      context?: string
  }
  ```
...
您可以通过客户端 API 传递 `extensions` 来使用 WebAuthn 扩展。当 `returnWebAuthnResponse` 为 true 时，客户端会返回 `webauthn.clientExtensionResults`。
