...
* **domain**: 你的应用程序域名（生成 SIWE 消息时必需）
* **emailDomainName**: 创建用户账户时的电子邮件域名（不使用匿名模式时）。默认为基础 URL 的域名
* **anonymous**: 是否允许匿名登录（无需电子邮件）。默认值为 `true`
* **getNonce**: 生成每个登录尝试的唯一 nonce 的函数。你必须实现此函数以返回加密安全的随机字符串。必须返回 `Promise<string>`
* **verifyMessage**: 验证已签名 SIWE 消息的函数。接收消息详情并应返回 `Promise<boolean>`
* **ensLookup**: 可选函数，用于查询以太坊地址的 ENS 名称和头像
