...
该软件包附带 22 种语言的现成翻译，因此你无需从头编写翻译。请从 `@better-auth/i18n` 导入 `locales` 以使用它们。
...
| `CREDENTIAL_ACCOUNT_NOT_FOUND` | 找不到凭据账户         |
...
<Callout type="info">
  `ctx.request` 对于非 HTTP 调用可能未定义。
</Callout>

## 回退行为

* 如果在检测到的语言环境中找不到某个特定错误代码的翻译，则保留内置英文信息。
* 如果检测到的语言环境不在翻译字典中，插件会回退到 `defaultLocale`。
* 如果任何策略都无法检测到语言环境，插件会回退到 `defaultLocale`（默认为 `"en"`）。
* 非错误响应不会被此插件修改。
