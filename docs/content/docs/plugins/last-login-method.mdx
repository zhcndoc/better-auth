    ...
    plugins: [
        lastLoginMethod({
            ...
            beforeStoreCookie: async (ctx, lastUsedLoginMethod) => {
                // 检查用户是否已同意非必要 Cookie
                // 返回 false 可阻止 Cookie 存储
                const hasConsent = await checkUserCookieConsent(ctx)
                return hasConsent
            },
            
            // Schema 自定义（当 storeInDatabase 为 true 时）
            schema: {
                user: {
...
* 是否将最后登录方式存储到数据库
* 默认: `false`
* 启用后，会向用户表添加一个 `lastLoginMethod` 字段
* 如需仅将方式存储在数据库中（不使用 Cookie），可与 `beforeStoreCookie: () => false` 结合：

```ts title="auth.ts"
lastLoginMethod({
    storeInDatabase: true,
    beforeStoreCookie: () => false, // 从不设置非必要 Cookie
})
```
...
* 在存储上次登录方式 Cookie 之前运行的钩子函数
* 返回 `true` 允许设置 Cookie，返回 `false` 则阻止设置
* 适用于 GDPR 合规或其他要求 Cookie 存储需用户同意的法规
* 如果函数抛出错误，Cookie 将不会被设置（会记录错误，但不会中断身份验证）
* **示例**：在存储 Cookie 前检查用户同意
...
export const authClient = createAuthClient({
    plugins: [
        lastLoginMethodClient({
            cookieName: "better-auth.last_used_login_method", // 默认: "better-auth.last_used_login_method"
            domain: ".example.com" // 跨子域清除 Cookie 所必需
        })
    ]
})
```
...
* 用于清除 Cookie 的域
* 在使用 `crossSubDomainCookies` 时必需，这样客户端才能正确清除服务端设置的 Cookie
* 应与服务端 `crossSubDomainCookies` 配置中的 `domain` 值匹配
...
* **邮箱认证**: `"email"`
* **OAuth 提供程序**: 提供程序 ID（例如 `"google"`, `"github"`, `"discord"`）
* **OAuth2 回调**: URL 路径中的提供程序 ID
* **注册方式**: 与登录方式相同
...
<Callout type="warn">
  当使用 `crossSubDomainCookies` 时，你必须向 `lastLoginMethodClient()` 传递 `domain` 选项，这样 `clearLastUsedLoginMethod()` 才能正确让 Cookie 过期。否则，浏览器会静默忽略清除请求，因为域名不匹配。
</Callout>
...
export const authClient = createAuthClient({
    plugins: [
        lastLoginMethodClient({
            domain: ".example.com" // 必须与服务端 crossSubDomainCookies 的 domain 一致 // [!code highlight]
        })
    ]
})
```
...
最后登录方式 cookie 在 GDPR 法规下被视为非必要 cookie。
为了符合 GDPR 及类似隐私法规，只有在用户明确同意后，你才应该存储此 cookie。

`beforeStoreCookie` 钩子允许你在存储 cookie 之前实现同意检查：
...
<Callout type="warn">
  当 `beforeStoreCookie` 返回 `false` 或抛出错误时，cookie 将不会被设置。认证仍然会正常成功 - 只会阻止 cookie 的存储。
</Callout>
