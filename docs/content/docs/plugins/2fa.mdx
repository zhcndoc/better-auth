...
注意：在用户验证其 TOTP 代码之前，`twoFactorEnabled` 不会被设置为 `true`。有关在 [TOTP 部分](#totp) 验证 TOTP 的更多信息。如果您在插件配置中将 `skipVerificationOnEnable` 设置为 true，则可以跳过验证。
