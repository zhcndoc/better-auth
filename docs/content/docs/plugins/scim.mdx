...
跨域身份管理系统（[SCIM](https://simplecloud.info/#Specification)）通过标准化协议，使多域场景中的身份管理更容易支持。  
此插件公开了一个符合规范的 [SCIM 2.0](https://simplecloud.info/#Specification) 服务器，允许第三方身份提供商将身份同步到您的服务。

<Callout>
  需要自助式 SCIM 设置，以便您的客户可以配置自己的身份提供商同步？[联系企业版](/enterprise)。
</Callout>
...
* **SCIM 承载令牌**：请参阅[生成 SCIM 令牌](#generating-a-scim-token)

### 自助目录同步

如果您使用 [Better Auth Infrastructure](https://dash.better-auth.com/sign-in)，在仪表板中会获得自助目录同步。组织管理员可以创建和管理 SCIM 目录连接，并在不直接调用 SCIM API 的情况下轮换承载令牌。
...
<Callout>
  **重要**：个人 SCIM 连接仍可由任何经过身份验证的用户生成。组织范围连接的默认限制是具有 `admin` 角色或组织创建者角色（`organization.creatorRole`，默认为 `owner`）的用户。如果需要不同的策略，请配置 [`requiredRole`](#options) 和/或在 [钩子](#hooks) 中添加更严格的检查。
</Callout>
...
* `admin`
* `organization.creatorRole` 或 `owner`
...
请参考 [架构](#如果通过providerownershipenabled启用了提供商所有权) 部分手动添加字段。
...
<APIMethod path="/scim/v2/Users" method="GET" requireBearerToken isExternalOnly note="返回配置的 SCIM 用户详细信息。请参考 https://datatracker.ietf.org/doc/html/rfc7644#section-3.4.1">
  ```ts
  type listSCIMUsers = {
      /**
       * SCIM 兼容的过滤表达式
...
<APIMethod path="/scim/v2/Users/:userId" method="GET" forceAsParam requireBearerToken isExternalOnly note="返回配置的 SCIM 用户详细信息。请参考 https://datatracker.ietf.org/doc/html/rfc7644#section-3.4.1">
  ```ts
  type getSCIMUser = {
      /**
       * 唯一用户标识符
...
<APIMethod path="/scim/v2/Users" method="POST" requireBearerToken isExternalOnly note="通过 SCIM 配置新用户。请参考 https://datatracker.ietf.org/doc/html/rfc7644#section-3.3">
  ```ts
  type createSCIMUser = {
      /*
       * 唯一的第三方（外部）标识符
...
<APIMethod path="/scim/v2/Users/:userId" method="PUT" requireBearerToken isExternalOnly note="通过 SCIM 更新已有用户。请参考 https://datatracker.ietf.org/doc/html/rfc7644#section-3.3">
  ```ts
  type updateSCIMUser = {
      /*
       * 唯一的第三方（外部）标识符
...
<APIMethod path="/scim/v2/Users/:userId" method="PATCH" requireBearerToken isExternalOnly note="部分更新用户资源。请参考 https://datatracker.ietf.org/doc/html/rfc7644#section-3.5.2">
  ```ts
  type patchSCIMUser = {
      /**
       * 必需的模式声明
...
移除一个用户资源。此操作仅影响属于与 SCIM 令牌相同提供商（以及组织）的用户。

对于组织范围的令牌，用户会从组织中取消分配：其成员资格和提供商账户会被移除，而全局用户记录会保留。对于非组织令牌，仅当该提供商账户是其唯一身份时才会删除全局用户；否则只会解除该账户的关联。

<APIMethod path="/scim/v2/Users/:userId" method="DELETE" forceAsParam requireBearerToken isExternalOnly note="删除现有的用户资源。请参考 https://datatracker.ietf.org/doc/html/rfc7644#section-3.6">
  ```ts
  type deleteSCIMUser = {
      userId: string
  }
...
<APIMethod path="/scim/v2/ServiceProviderConfig" method="GET" isExternalOnly note="标准 SCIM 元数据端点，由身份提供商使用。请参考 https://datatracker.ietf.org/doc/html/rfc7644#section-4">
  ```ts
...
<APIMethod path="/scim/v2/Schemas" method="GET" isExternalOnly note="标准 SCIM 元数据端点，由身份提供商使用以获取支持的模式信息。请参考 https://datatracker.ietf.org/doc/html/rfc7644#section-4">
  ```ts
  type getSCIMSchemas = {
  }
  ```
...
<APIMethod path="/scim/v2/Schemas/:schemaId" method="GET" isExternalOnly note="标准 SCIM 元数据端点，由身份提供商使用以获取给定模式的信息。请参考 https://datatracker.ietf.org/doc/html/rfc7644#section-4">
  ```ts
  type getSCIMSchema = {
  }
  ```
...
<APIMethod path="/scim/v2/ResourceTypes" method="GET" isExternalOnly note="标准 SCIM 元数据端点，由身份提供商使用以获取服务器支持的类型列表。请参考 https://datatracker.ietf.org/doc/html/rfc7644#section-4">
  ```ts
...
<APIMethod path="/scim/v2/ResourceTypes/:resourceTypeId" method="GET" isExternalOnly note="标准 SCIM 元数据端点，由身份提供商使用以获取服务器支持的类型详情。请参考 https://datatracker.ietf.org/doc/html/rfc7644#section-4">
  ```ts
...
* `user.name`：从 `name`（`name.formatted` 或 `name.givenName` + `name.familyName`）派生，如果没有则回退到用户的主电子邮件
* `account.providerId`：与 `SCIM` 令牌关联的提供商
* `account.accountId`：默认为 `externalId`，如果没有则回退到 `userName`
* `member.organizationId`：与提供商关联的组织

SCIM 的 `active` 属性映射到用户的禁用状态。`active: false` 会停用用户（通过 [admin](/docs/plugins/admin) 插件的 `banned` 状态）并撤销其会话；`active: true` 会重新启用。要生效 `active` 需要 admin 插件。通过 SCIM 更改用户邮箱也会重置其已验证状态。
...
* `providerOwnership`: `{ enabled: boolean }` — 当启用时，将每个个人提供商连接与生成其令牌的用户关联。详见[连接所有权](#scim-提供商连接所有权)。默认为 `{ enabled: false }`。
...
* `storeSCIMToken`: 存储 SCIM 令牌的方法，可以是 `encrypted`、`hashed` 或 `plain` 文本。默认为 `plain` 文本。
