...
| `allowIdpInitiated`            | `boolean` | `true`           | 允许 IdP 发起的 SSO（没有 InResponseTo 的响应）。若需更严格安全性可设为 `false`。仅在启用校验时适用。 |
| `requestTTL`                   | `number`  | `300000` (5 min) | AuthnRequest 记录的生存时间，单位毫秒。超过该时间的请求将被拒绝。                            |
...
<DatabaseTable
  fields={[
      ...
      { name: "issuer", type: "string", description: "发行者标识符" },
      { name: "domain", type: "string", description: "提供者域名" },
      { name: "oidcConfig", type: "string", description: "OIDC 配置（JSON 字符串）", isOptional: true },
      { name: "samlConfig", type: "string", description: "SAML 配置（JSON 字符串）", isOptional: true },
      { name: "userId", type: "string", description: "用户 ID", isForeignKey: true },
      { name: "providerId", type: "string", description: "提供者 ID。用于识别提供者并生成重定向 URL。", isUnique: true },
      { name: "organizationId", type: "string", description: "组织 ID。如果提供者关联到某个组织。", isOptional: true }
  ]}
/>
...
<DatabaseTable
  fields={[
      { name: "domainVerified", type: "boolean", description: "指示提供者域名是否已验证的标志。", isOptional: true },
  ]}
/>
...
<TypeTable
  ...
  provisionUserOnEveryLogin: {
      description: "如果为 true，则每次登录都会调用 provisionUser 回调，而不只是新用户注册时。",
      type: "boolean",
      default: false,
  },
  organizationProvisioning: {
  ...
  saml: {
      ...
      properties: {
          enableInResponseToValidation: {
              description: "启用 SP 发起的 SAML 流程的 InResponseTo 验证。",
              type: "boolean",
              default: true,
          },
          allowIdpInitiated: {
