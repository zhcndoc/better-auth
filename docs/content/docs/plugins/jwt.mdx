...
<Callout type="info">
  JWT 插件的 `/token` 端点和 `set-auth-jwt` 头会生成可由 JWKS 验证的 JWT，供外部服务使用。它们与 Better Auth 的主 `session_token` cookie 相互独立。

  如果启用 `session.cookieCache.strategy = "jwt"`，`session_data` cookie 默认仍使用 Better Auth 密钥和 HS256。若要让该 cookie-cache JWT 也能通过 JWT 插件的 JWKS 进行验证，请在 `jwt()` 上设置 `sessionCookieCache: true`。
</Callout>
...
将 `sessionCookieCache` 设置为 `true`，使用插件本地管理的密钥对会话 Cookie 缓存值进行签名。这要求将 `session.cookieCache.strategy` 设置为 `"jwt"`。
