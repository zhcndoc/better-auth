...
<Callout>
  当链接验证了一个已存在的账户，而该账户的邮箱从未被确认时，该账户上现有的密码将被移除，其会话也会被撤销。用户会通过该链接登录，并可通过密码重置设置新密码。这将以链接所证明的邮箱所有权作为账户的事实依据。
</Callout>
...
**allowedAttempts** (deprecated): 每次验证调用现在都会在首次尝试时以原子方式消耗令牌，因此无论此设置如何，重试都会始终失败并返回 `?error=INVALID_TOKEN`。保留此选项仅为保持源码兼容性，但会被忽略；不再支持多次尝试兑换。将其设置为除 `1` 之外的任何值都会在启动时发出 `console.warn`（包括 `0`，它以前会立即拒绝，而现在没有任何效果）。
...
<Callout type="warn">
  当 `secondaryStorage` 承载验证（`verification.storeInDatabase: false`）时，原子性的单次使用保证要求你的 secondary storage 提供 `getAndDelete`（Redis `GETDEL`、KV `getAndDelete`）。如果实现只提供 `get` 和 `delete`，则消耗过程会回退到进程内的 JavaScript 锁，而该锁无法在多个应用实例之间协调。使用 secondary-storage 验证的多实例部署必须配置实现了 `getAndDelete` 的后端。
</Callout>
