...
**allowedAttempts** (已弃用)：现在每次验证调用都会在首次尝试时以原子方式消费令牌，因此无论此设置如何，重试都会始终失败并返回 `?error=INVALID_TOKEN`（参见 [GHSA-hc7v-rggr-4hvx](https://github.com/better-auth/better-auth/security/advisories/GHSA-hc7v-rggr-4hvx)）。该选项仅为保持源码兼容而保留，但会被忽略；不再支持多次尝试兑换。将其设置为除 `1` 之外的任何值都会在启动时发出 `console.warn`（包括 `0`，它以前会立即拒绝，而现在不再生效）。
...
<Callout type="warn">
  当 `secondaryStorage` 承载验证（`verification.storeInDatabase: false`）时，来自 [GHSA-hc7v-rggr-4hvx](https://github.com/better-auth/better-auth/security/advisories/GHSA-hc7v-rggr-4hvx) 的原子性单次使用保证要求你的 secondary storage 提供 `getAndDelete`（Redis `GETDEL`，KV `getAndDelete`）。如果实现只提供 `get` 和 `delete`，则消费操作会回退到进程内的 JavaScript 锁，而该锁无法在多个应用实例之间协调。使用 secondary-storage 验证的多实例部署必须配置一个实现了 `getAndDelete` 的后端。
</Callout>
