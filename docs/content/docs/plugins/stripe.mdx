...
<Steps>
  ...
  <Step>
    ...
    const stripeClient = new Stripe(process.env.STRIPE_SECRET_KEY!, {
        apiVersion: "2026-06-24.dahlia", // 截至 Stripe SDK v22.0.0 的最新 API 版本
    })
...
| `plans`                    | `StripePlan[]` 或 `function` | 订阅计划数组，或返回计划的异步函数。启用时**必需**。                                                                                              |
...
Stripe 插件集成了 [组织插件](/docs/plugins/organization)，支持组织作为 Stripe 客户。订阅的计费对象由个人用户转为组织，适合 B2B 场景。
