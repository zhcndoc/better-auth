...
1. **限流**：插件会强制执行轮询间隔以防止滥用
2. **代码过期**：设备码和用户码会在配置的时间后过期（默认：30 分钟）
3. **客户端验证**：在生产环境中始终验证 client ID，以防止未授权访问
4. **仅限 HTTPS**：在生产环境中进行设备授权时始终使用 HTTPS
5. **用户码格式**：用户码使用有限的字符集（不包含容易混淆的字符，如 0/O、1/I）以减少输入错误
6. **需要身份验证**：调用 `GET /device` 时用户必须已通过身份验证。验证步骤会为调用会话认领待处理的设备码，之后只有该会话可以批准或拒绝它
7. **预绑定**：使用 `user_id` 签发的设备码会跳过认领步骤，并且只能由该用户批准或拒绝。仅从受信任的服务端代码传递 `user_id`
