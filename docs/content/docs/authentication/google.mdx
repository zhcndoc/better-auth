...
Google 会在同一个 Google Cloud 项目中为每个平台分别发放一个 Client ID。将数组传递给 `clientId`，即可接受来自任一平台的 ID Token。有关共享提供者选项语义，请参阅 [clientId](/docs/concepts/oauth#clientid)。
...
有关底层参数行为，请参阅 Google 的[增量授权](https://developers.google.com/identity/protocols/oauth2/web-server#incrementalAuth)文档。
