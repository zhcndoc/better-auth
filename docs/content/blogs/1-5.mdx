...
<Callout type="warn">
  **Better Auth 2.x：** 这些 1.5 客户端和适配器 API 已被移除。请参阅 [MCP 升级指南](/docs/guides/1-7-upgrade-guide#mcp) 和 [当前的 MCP 插件文档](/docs/plugins/mcp)。
</Callout>
...
<Callout type="info">
  OAuth 2.1 Provider 适用于 OAuth 和 OpenID Connect 授权服务器集成。MCP 集成使用相同的提供者基础。
</Callout>
...
[👉 进一步了解动态基础 URL](https://www.better-auth.com/docs/guides/dynamic-base-url)
...
| Organization `permission` field                                 | `permissions`（复数）                        |
