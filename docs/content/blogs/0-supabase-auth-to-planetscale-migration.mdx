...
最近，[PlanetScale 宣布](https://planetscale.com/blog/planetscale-for-postgres)支持 PostgreSQL。这是一个令人振奋的消息，也是数据库行业的重要进步。
...
<Steps>
  <Step>
    打开 PlanetScale [控制面板](https://app.planetscale.com/)
  </Step>
  ...
  <Step>
    将数据库 URL 保存到你的 `.env` 文件中，以便稍后与 Better Auth 一起使用：
...
<Steps>
  ...
  <Step>
    按照并完成[基础设置](https://www.better-auth.com/docs/installation)
  </Step>
</Steps>
...
<Steps>
  <Step>
    运行以下 CLI 命令以生成设置 Better Auth 所需的所有模式：
  ...
  <Step>
    然后运行以下命令将生成的模式应用到你的 PlanetScale
    数据库：
...
<Callout type="success">
  现在你应该已经在 PlanetScale 中拥有了所需的认证表。
</Callout>
...
如果你在 Supabase 中还有其他与用户相关的数据，可以使用[Supabase 到 PlanetScale 的迁移工具](https://planetscale.com/docs/postgres/imports/supabase-import)。
...
* 确保在生产环境中检查所有环境变量是否已设置。
* 在正式上线前测试所有认证流程（注册、登录、密码重置、会话刷新）。
* 请记住，这只是基础内容；如果你在很多地方集成了 Supabase Auth 的认证函数，你需要找到合适的[Better Auth 替代方案](https://www.better-auth.com/docs)。
* 玩得开心！

### 了解更多！

<Cards>
  <Card href="https://www.better-auth.com/docs/introduction" title="Better Auth Setup">
    从安装 Better Auth 开始
  </Card>
  ...
  <Card href="https://planetscale.com/docs/postgres/imports/postgres-imports" title="PlanetScale Migration Guides">
    使用本指南将你的数据从 Supabase 及更多服务迁移过来
  </Card>

  <Card href="https://www.better-auth.com/docs/guides/supabase-migration-guide" title="Supabase Auth Migration">
    将你的认证从 Supabase Auth 迁移到你自己的数据库
  </Card>
</Cards>
