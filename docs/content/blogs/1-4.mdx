---
title: "Better Auth 1.4"
description: "无状态认证、SCIM、数据库查询优化、OAuth 流程中的额外状态、性能提升、SSO 域验证等等！"
date: 2025-11-21
author:
  name: "Bereket Engida"
  avatar: "/avatars/beka.jpg"
  twitter: "bekacru"
tags: ["1.4", "stateless sessions", "SCIM", "database indexing", "database join", "SSO domain verification", "uuid", "device authorization"]
---

## Better Auth 1.4 发布

我们很高兴宣布 Better Auth 1.4 版本发布。本次发布包含多项新功能、性能及安全改进。

升级请运行：

```package-install
npm install better-auth@1.4
```

***

## 🚀 重点功能

### 无状态认证

现在只需在认证配置中省略 `database` 选项，即可在不使用任何数据库的情况下启用无状态会话管理。

[👉 详情请阅读无状态认证](/docs/concepts/session-management#stateless-session-management)

```ts
import { betterAuth } from "better-auth";

export const auth = betterAuth({
    // 无数据库配置
    socialProviders: {
        google: {
            clientId: process.env.GOOGLE_CLIENT_ID,
            clientSecret: process.env.GOOGLE_CLIENT_SECRET,
        },
    },
});
```

此外，还支持在无数据库环境下使用 `accessToken`、`accountInfo` 和 `refreshToken` 端点。

```ts
import { authClient } from "@/lib/auth-client";

const accessToken = await authClient.getAccessToken();

// 获取账户信息
const accountInfo = await authClient.accountInfo();
```

***

SCIM 通过标准协议简化多域场景下身份管理的支持。

[👉 详情请阅读 SCIM 配置](/docs/plugins/scim)

```ts
import { scim } from "@better-auth/scim";
import { betterAuth } from "better-auth";

export const auth = betterAuth({
    plugins: [scim()]
});
```

***

现在可以在 OAuth 流程中传递任意额外的状态数据。

[👉 详情请阅读 OAuth 流程中的自定义状态](/docs/concepts/oauth#passing-additional-data-through-oauth-flow)

```ts
import { authClient } from "@/lib/auth-client";

await authClient.signIn.social({
    provider: "google",
    additionalData: { // [!code highlight]
        referralCode: "ABC123", // [!code highlight]
        source: "landing-page", // [!code highlight]
    }, // [!code highlight]
});
```

可在任意钩子、中间件或端点中访问这些额外数据：

```ts title="auth.ts"
import { betterAuth } from "better-auth";
import { getOAuthState, createAuthMiddleware } from "better-auth/api";

const auth = betterAuth({
    hooks: {
      before: createAuthMiddleware(async (ctx) => {
        const additionalData = await getOAuthState<{ referralCode: string, source: string }>();
        console.log(additionalData);
      })
    }
});
```

***

### 数据库连接改进

启用方法：在认证配置的 `experimental` 中设置 `joins` 为 `true`。
然后重新运行迁移或架构生成以使用支持连接的更新架构。

启用方法：在认证配置中将 `advanced.database.joins` 设置为 `true`。
然后重新运行迁移或架构生成，以获取支持连接的更新架构！

[👉 详细了解数据库连接](/docs/concepts/database#joins)

```ts
import { betterAuth } from "better-auth";

export const auth = betterAuth({
    advanced: {
        database: {
            joins: true,
        },
    },
});
```

<Callout type="info">
  所有适配器均支持连接。禁用时（默认设置），Better Auth 会回退到单独查询。
</Callout>

***

### API 密钥支持二级存储

API-Key 插件新增对二级存储的支持，加快密钥查询速度！

[👉 了解 API 密钥二级存储的更多信息](/docs/plugins/api-key#storage-modes)

```ts
import { apiKey } from "@better-auth/api-key";
import { betterAuth } from "better-auth";

export const auth = betterAuth({
    secondaryStorage: {
        //...
    },
    plugins: [apiKey({ storage: "secondary-storage" })], // [!code highlight]
});
```

<Accordions type="single">
  <Accordion title="迁移指南">
    1. 开始时启用 `fallbackToDatabase`，且 `storage` 设置为 `"secondary-storage"`：

    ```ts
    import { apiKey } from "@better-auth/api-key";
    import { betterAuth } from "better-auth";

    export const auth = betterAuth({
        secondaryStorage: {
            //...
        },
        plugins: [apiKey({ storage: "secondary-storage", fallbackToDatabase: true })],
    });
    ```

    这样数据库中已有的密钥仍可访问，新密钥则存储于二级存储中。

    2. **迁移现有密钥**（可选）：为了更快完成初始缓存填充，你可以编写迁移脚本
       将数据库中的所有密钥复制到二级存储中。不过这一步是可选的，因为密钥在被访问时缓存会自动预热。

    3. **禁用回退**：当所有密钥迁移完毕或缓存已充分填充后：

    ```ts
    import { apiKey } from "@better-auth/api-key";
    import { betterAuth } from "better-auth";

    export const auth = betterAuth({
        plugins: [
            apiKey({
                storage: "secondary-storage",
                fallbackToDatabase: false // 可省略，false 或 undefined // [!code highlight]
            })
        ],
    });
    ```
  </Accordion>
</Accordions>

***

### SSO 域验证

域验证可让应用自动信任新的 SSO 提供者，通过自动验证关联域的所有权来实现：

[👉 了解关于错误页面改进的更多信息](/docs/reference/options#onapierror)

<img src="/images/blogs/1-4_error.png" alt="错误页面" className="w-full max-w-2xl mx-auto rounded-lg border border-border" />

***

### SSO 域验证

域验证可让你的应用通过自动验证关联域的所有权，自动信任新的 SSO 提供者：

👉 [了解关于域验证的更多信息](/docs/plugins/sso#domain-verification)

```ts
import { sso } from "@better-auth/sso";
import { betterAuth } from "better-auth";

export const auth = betterAuth({
    plugins: [sso({ domainVerification: { enabled: true } })],
});
```

***

### CLI 支持数据库索引

Better-Auth CLI 现在内置支持数据库索引，大大提升应用性能。

```bash
npx auth generate

npx auth migrate
```

***

### JWT 密钥轮换支持

JWT 插件现支持密钥轮换，允许无停机地替换 JWT 密钥。

```ts
import { jwt } from "@better-auth/jwt";
import { betterAuth } from "better-auth";

export const auth = betterAuth({
    plugins: [
      jwt({ 
        jwks: { rotationInterval: 60 * 60 * 24 * 30, gracePeriod: 60 * 60 * 24 * 30 } // [!code highlight]
      })
    ],
});
```

👉 [了解关于 JWT 密钥轮换的更多信息](/docs/plugins/jwt#key-rotation)

### 改进的通用 OAuth 插件

尚未原生支持的 OAuth 提供者将作为预配置选项加入通用 OAuth 插件，计划最终支持 Auth.js 的所有提供者。

```ts
import { betterAuth } from "better-auth";
import { genericOAuth } from "better-auth/plugins";
import { keycloak } from "better-auth/plugins/generic-oauth";

export const auth = betterAuth({
    plugins: [genericOAuth({
        config: [
            keycloak({
                clientId: process.env.KEYCLOAK_CLIENT_ID,
                clientSecret: process.env.KEYCLOAK_CLIENT_SECRET,
                issuer: process.env.KEYCLOAK_ISSUER,
            })
        ]
    })
    ]
});
```

***

### 包体积优化

如果使用自定义适配器（如 Prisma、Drizzle 或 MongoDB），\
可通过使用 `better-auth/minimal` 替代 better-auth 来减小包体积。\
该版本不包含 Kysely，仅在直接数据库连接时需要。

[👉 详情请阅读包体积优化](/docs/guides/optimizing-for-performance#bundle-size-optimization)

```ts
import { drizzleAdapter } from "better-auth/adapters/drizzle";
import { betterAuth } from "better-auth/minimal"; // [!code highlight]
import { db } from "./database";

export const auth = betterAuth({
    database: drizzleAdapter(db, { provider: "pg" }),
});
```

***

### UUID 支持

现在开箱即用支持将 UUID 作为主 ID 字段类型。

[👉 详情请阅读 UUID 支持](/docs/concepts/database#uuids)

```ts
import { betterAuth } from "better-auth";
import { db } from "./database";

export const auth = betterAuth({
    database: db,
    advanced: {
        database: { generateId: "uuid" }, // [!code highlight]
    },
});
```

***

### 改进的邮箱更改流程

为增强安全性，现在可以要求用户先通过其**当前**邮箱确认更改，然后才会向新地址发送验证邮件。

[👉 详情请阅读改进的邮箱更改流程](/docs/concepts/users-accounts#confirming-with-current-email)

```ts
import { betterAuth } from "better-auth";

export const auth = betterAuth({
    user: {
        changeEmail: {
            enabled: true,
            sendChangeEmailConfirmation: async ({ user, newEmail, url, token }, request) => {
                sendEmail({
                    to: user.email, // 发送到当前邮箱
                    subject: '批准邮箱更改',
                    text: `点击链接以批准更改至 ${newEmail}：${url}`
                })
            }
        }
    }
});
```

***

### 设备授权插件

完整支持 OAuth 2.0 设备授权协议（RFC 8628），允许用户在输入受限设备（智能电视、游戏机等）上登录。

[👉 详情请阅读设备授权](/docs/plugins/device-authorization)

```ts
import { betterAuth } from "better-auth";
import { deviceAuthorization } from "better-auth/plugins";

export const auth = betterAuth({
    plugins: [
        deviceAuthorization()
    ]
});
```

***

### 基于 Cookie 的账户存储

现在支持将用户账户数据存储于签名 Cookie 中，非常适合无状态应用或延迟数据库持久化。

```ts
export const auth = betterAuth({
    account: {
        storeAccountCookie: true
    } 
});
```

***

建议逐项检查破坏性变更，确保升级顺利。

### 现有认证流程

小幅调整以提升流畅度、速度和可靠性。

* **邮箱更改流程**：移除 `changeEmail` 流程中的 `sendChangeEmailVerification`，改用 `emailVerification.sendVerificationEmail`。
* **忘记密码流程**：将 `authClient.forgotPassword` 改名为 `authClient.requestPasswordReset`。
* **账户信息端点**：`accountInfo` 端点从 `POST /account-info` 改为 `GET /account-info`。

- **邮箱更改流程**：已从 `changeEmail` 流程中移除 `sendChangeEmailVerification`，改为使用 `emailVerification.sendVerificationEmail`。
- **忘记密码流程**：`authClient.forgotPassword` 现在改为 `authClient.requestPasswordReset`。
- **账户信息端点**：`accountInfo` 端点已从 `POST /account-info` 改为 `GET /account-info`。

  这意味着所有 `auth.api` 调用都需要从：

  ```ts
  auth.api.accountInfo({
      body: {
          // ...
      }
  });
  ```

  改为：

  ```ts
  auth.api.accountInfo({
      query: {
          // ...
      }
  });
  ```

### 插件选项变更

多个插件回调中的 `request` 参数改为 `ctx`。\
Passkey 插件独立成包，API Key 的模拟会话默认关闭。

* **Passkey 插件**：Passkey 插件现已移至独立包。

  立即安装：

  ```package-install
  npm install @better-auth/passkey
  ```

* **Email OTP 插件**：`sendVerificationOTP` 和 `generateOTP` 的插件选项现在在其回调函数中使用 `ctx` 而不是 `request`。你仍然可以通过 `ctx.request` 访问请求。

* **Magic Link 插件**：`sendMagicLink` 的插件选项现在在其某个回调选项中使用 `ctx` 而不是 `request`。你仍然可以通过 `ctx.request` 访问请求。

* **电话号码插件**：`sendOTP`、`sendPasswordResetOTP` 和 `callbackOnVerification` 的插件选项现在在其某个回调选项中用 `ctx` 替代了 `request`。你仍然可以通过 `ctx.request` 访问请求。

* **组织插件**：`teams.defaultTeam.customCreateDefaultTeam` 和 `teams.maximumTeams` 的插件选项现在在其某个回调选项中用 `ctx` 替代了 `request`。你仍然可以通过 `ctx.request` 访问请求。

* **OIDC 插件**：`redirectURLs` 字段已更新为 `redirectUrls`，这需要进行数据库迁移。

* **API Key 插件**：基于 api-key 的模拟会话现在默认禁用，应先通过认证配置显式启用。

### 认证配置变更

数据库配置选项调整。

* **已移除 `advanced.generateId`**：长期弃用的 `advanced.generateId` 已被移除。请改用 `advanced.database.generateId`。
* **`advanced.database.useNumberId` 已迁移**：`advanced.database.useNumberId` 已迁移到 `advanced.database.generateId: "serial"`。

### Tanstack Start

Better-Auth 提供的 `reactStartCookies` 插件现已迁移为 `tanstackStartCookies` 插件。

```diff
- import { reactStartCookies } from "better-auth/react-start";
+ import { tanstackStartCookies } from "better-auth/tanstack-start";

export const auth = betterAuth({
-    plugins: [reactStartCookies()],
+    plugins: [tanstackStartCookies()],
});
```

### Expo

Expo 上的 `useSession` 新增轮询特性，需要安装 `expo-network`：

```package-install
npx expo install expo-network
```

### Cloudflare 运行时

Better Auth 使用 AsyncLocalStorage 进行异步上下文跟踪。\
在 Cloudflare Workers 需于 `wrangler.toml` 中添加 `nodejs_compat` 标记以启用：

```toml title="wrangler.toml"
compatibility_flags = ["nodejs_compat"]
compatibility_date = "2024-09-23"
```

或者，如果你只需要 AsyncLocalStorage 支持：

```toml title="wrangler.toml"
compatibility_flags = ["nodejs_als"]
```

下一大版本将默认假设支持 AsyncLocalStorage，此配置届时必需。

***

## ✨ 更多功能

* **Cookie 分块**: 不再出现 Cookie 大小超限错误。
* **MongoDB 字符串 ID**: 支持在 MongoDB 中使用字符串 ID 而非 ObjectID。
* **JWE Cookie 缓存**: 会话存储 Cookie 缓存默认使用 JWE 以增强安全性。
* **OIDC RP 启动注销**: 在 OIDC 提供程序触发注销。
* **服务端 IP 检测**: 自动客户端 IP 检测工具。
* **自定义 OTP 验证**: 允许在电话号码插件中使用自定义逻辑进行 OTP 验证。
* **客户端 `disableSignal`**: 可选项，用于禁用默认的中止信号行为。
* **`AuthClient` 类型助手**: 新的类型助手，用于更好的客户端类型推断。
* **新提供者**: 添加了 Polar 和 Paybin OAuth 提供者。
* **仅 ESM**: Better-Auth 现在完全支持 ESM。
* **会话重新获取**: 支持轮询和窗口聚焦时重新获取。
* **Cloudflare Workers**: CLI 支持虚拟模块导入。
* **错误文档**: 常见的 OAuth 流程错误现已 [记录在案](/docs/reference/errors)。
* **最后登录方式**: 支持跟踪 SIWE 和密钥登录。

***

## 🐛 错误修复与改进

本次发布共修复 250 多个问题，涵盖多个领域：

* **适配器修复**: 改进查询处理、更佳的类型转换 UUID 列类型，并解决了可为空的外键问题。
* **会话管理**: 修复会话刷新触发器，改进 Cookie 处理及缓存管理。
* **OAuth**: 修复状态管理，改进回调处理，并解决跨来源问题。
* **组织**: 修复成员删除、团队管理、权限检查和推断问题。
* **双因素认证**: 改进 TOTP 错误信息，修复备份代码加密，并增强设备信任处理。
* **电子邮件 OTP**: 防止重复发送验证邮件，并改进会话信号触发。
* **SSO**: 修复 SAML 回调处理，改进 OIDC 作用域回退，并增强同意强制执行。
* **API 密钥**: 修复使用量跟踪，并改进请求验证。
* **电话号码**: 防止未经授权的电话号码更新，并改进验证流程。
* 以及更多！

***

大量细节优化让一切更顺畅、更快速、更可靠。\
👉 [查看完整更新日志](https://github.com/better-auth/better-auth/releases/tag/v1.4.0)

## ❤️ 贡献者

感谢所有贡献者使本次发布成为可能！

export const releaseContributorUsernames = [
	// cspell:disable
	"zain",
	"dandamian",
	"hartbit",
	"rmarscher",
	"kaandok",
	"acusti",
	"rbayliss",
	"kevcube",
	"bsklaroff",
	"jonathansamines",
	"mburumaxwell",
	"erquhart",
	"dmmulroy",
	"iRoachie",
	"mohebifar",
	"thomasmol",
	"borgoat",
	"XiNiHa",
	"Berndwl",
	"xiaoyu2er",
	"ebalo55",
	"udnes99",
	"himself65",
	"dvanmali",
	"AlexStrNik",
	"ThibautCuchet",
	"zbeyens",
	"xuchenhao001",
	"tnkuehne",
	"Tobix99",
	"hieudien14310",
	"3ddelano",
	"bdkopen",
	"jslno",
	"chhoumann",
	"mrl5",
	"tgrassl",
	"hyoban",
	"redoh",
	"gregtjack",
	"ouwargui",
	"kanarian",
	"natetewelde",
	"Badbird5907",
	"max-programming",
	"eni4sure",
	"frectonz",
	"ephraimduncan",
	"Eazash",
	"ahmedriad1",
	"Velka-DEV",
	"Kinfe123",
	"zy1p",
	"okisdev",
	"surafel58",
	"DevDuki",
	"TheUntraceable",
	"nakasyou",
	"JagritGumber",
	"HoshangDEV",
	"vagxrth",
	"Paola3stefania",
	"Bekacru",
	"kira-1011",
	"almadoro",
	"ocherry341",
	"QuintenStr",
	"Ridhim-RR",
	"ahmed-abdat",
	"mohit4bug",
	"Pankaj3112",
	"ShobhitPatra",
	"ping-maxwell",
	"nbifrye",
	"rovertrack",
	"GautamBytes",
	"AntonVishal",
	"bytaesu",
	"ic4l4s9c",
	"jcajuab",
	"yutaka5",
	// cspell:enable
];

<Contributors usernames={releaseContributorUsernames} />
